...
import {setBackendWorkerSession} from '@lib/backendWorkerSession';
...
class ApiManagerProxy extends MTProtoMessagePort {
  ...
  public sendEnvironment() {
    ...
    this.pushBackendSessionToWorker();
  }
  ...
  private _registerServiceWorker() {
    ...
    fetch(ServiceWorkerURL, {method: 'GET'})
    .then((response) => {
      if(!response.ok) {
        throw new Error(`ServiceWorker script is unavailable (${response.status})`);
      }
  ...
  /** Sync backend URL + token to shared worker and main thread (WebSocket uses main-thread session). */
  public pushBackendSessionToWorker() {
    if(!Modes.backend) {
      return;
    }
    ...
    setBackendWorkerSession({token, baseUrl});
    this.invokeVoid('backendSession', {token, baseUrl});
  }
  ...
  public sendAllStates(loadedStates: Awaited<ReturnType<ApiManagerProxy['loadAllStates']>>) {
    ...
    const flushed = Promise.all(promises);
    if(!Modes.backend) {
      return flushed;
    }
    ...
    return flushed.then(async() => {
      try {
        const {runBackendMainDataBootstrap} = await import('@lib/backendBootstrapMain');
        await runBackendMainDataBootstrap();
      } catch(err) {
        console.error('[backend] runBackendMainDataBootstrap failed', err);
      }
    });
  }
