...
import {
  buildDialogsSliceFromBackendChatRows,
  buildMessageFromBackend,
  mapBackendUser
} from '@lib/backendMtprotoAdapter';
import {backendUuidToChatPeerId} from '@lib/backendPeerIds';
...
export class AppMessagesManager extends AppManager {
  ...
  /**
   * Apply WebSocket bootstrap payloads (`getCurrentUser` + `getChats`) into dialogs/users/messages mirrors.
   */
  public hydrateBackendWsBootstrap(strictUser: Record<string, unknown>, strictChats: unknown[]) {
    if(!Modes.backend) {
      return;
    }

    const uid = typeof strictUser.id === 'string' ? strictUser.id : '';
    if(!uid) {
      return;
    }
    ...
    const meRow = {
      ...
      profile: {
        ...
        avatarUrl: strictUser.avatar,
        status: strictUser.status,
        firstName: (typeof strictUser.username === 'string' && strictUser.username.trim()) ?
          strictUser.username.trim() :
          'Account',
        lastName: ''
      },
    ...
    const rows = (Array.isArray(strictChats) ? strictChats : []).map((c: any) => ({
      id: c.id,
      title: c.title || 'Chat',
      type: c.type,
      createdAt: new Date(),
      members: Array.isArray(c.members) ? c.members : [] as {user?: unknown}[],
      lastMessage: c.lastMessage ? {
        id: `lm-${c.id}`,
        senderId: c.lastMessage.senderId || uid,
        text: c.lastMessage.text || '',
        createdAt: new Date(typeof c.lastMessage.createdAt === 'number' ?
    ...
    const slice = buildDialogsSliceFromBackendChatRows(rows, meRow, this.apiManager);
    this.dialogsStorage.applyDialogs(slice as any);
    this.dialogsStorage.setDialogsLoaded(0, true);
  }

  /**
   * Merge REST-loaded messages into active history (backend mode).
   */
  public applyBackendFetchedMessages(chatBackendUuid: string, prismaRows: unknown[]) {
    if(!Modes.backend || typeof chatBackendUuid !== 'string' || !Array.isArray(prismaRows)) {
      return;
    }

    const peerId = backendUuidToChatPeerId(chatBackendUuid);
    const storage = this.getHistoryMessagesStorage(peerId);

    for(const raw of prismaRows) {
      const row = raw as Record<string, unknown>;
      const sender = row.sender as Record<string, unknown> | undefined;
      if(sender && typeof sender === 'object') {
        this.appUsersManager.saveApiUsers([mapBackendUser(sender, {})]);
      } else if(typeof row.senderId === 'string') {
        const sid = row.senderId;
        this.appUsersManager.saveApiUsers([mapBackendUser({
          id: sid,
          profile: {firstName: sid.slice(0, 16), username: sid.slice(0, 32)}
        }, {})]);
      }
    }

    const msgs = prismaRows.map((raw) => {
      const row = raw as Record<string, unknown>;
      return buildMessageFromBackend({
        id: row.id,
        senderId: row.senderId,
        text: row.text,
        createdAt: row.createdAt
      }, chatBackendUuid, this.apiManager);
    });

    if(msgs.length) {
      this.saveMessages(msgs, {storage});
    }
  }
