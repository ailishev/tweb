...
import {connectBackendRealtime} from '@lib/backendRealtime';
...
class ApiUpdatesManager {
  ...
  private processUpdate(update: Update, options: Partial<{
    ...
    } else if(!Modes.backend && channelId && !this.appChatsManager.hasChat(channelId)) {
    ...
    if(pts) {
      const backendPts = Modes.backend && !channelId;
      if(backendPts) {
        if(pts > curState.pts) {
          curState.pts = pts;
          popPts = true;
          curState.lastPtsUpdateTime = Date.now();
        } else if(pts_count && pts === curState.pts) {
  ...
  public attach() {
    ...
    if(Modes.backend) {
      this.updatesState.seq = 1;
      this.updatesState.pts = 1000;
      this.updatesState.date = Math.floor(Date.now() / 1000);
      this.updatesState.syncLoading = null;
      this.saveUpdatesState();

      this.apiManager.setUpdatesProcessor(this.processUpdateMessage);
      this.setProxy();
      connectBackendRealtime(this);
      return;
    }
