...
export class AppUsersManager extends AppManager {
  ...
  private ensureUserForInput(id: UserId): MTUser.user {
    let user = this.getUser(id);
    if(user) {
      return user;
    }

    // Backend adapter can request input peers before the user cache is hydrated.
    user = {
      _: 'user',
      pFlags: {},
      id,
