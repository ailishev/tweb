...
export class WebPushApiManager extends EventListenerBase<{
  ...
  /** Set when ApiManagerProxy registers the service worker; absent in backend / no-SW modes. */
  ...
  public start() {
    if(apiManagerProxy.serviceMessagePort) {
      this.serviceMessagePort = apiManagerProxy.serviceMessagePort;
    }
    if(!this.isAvailable || !this.serviceMessagePort) {
      return;
    }
  ...
  public isAliveNotify = async() => {
    if(!this.isAvailable || singleInstance.deactivatedReason || !this.serviceMessagePort) {
      return;
    }
