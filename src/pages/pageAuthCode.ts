...
const submitCode = (code: string) => {
  ...
  try {
    sessionStorage.setItem('backend_last_otp_code', code);
  } catch(err) {}
  ...
  const params: {
    ...
    phone_code: code,
    verify_token: (authSentCode as any).verify_token
  };

  backendAuthApi.verifyCode(params)
  .then(async(result) => {
    if(!result.ok) {
      throw {type: result.error || 'VERIFY_CODE_FAILED'};
    }
    ...
    throw {type: 'INVALID_VERIFY_RESPONSE'};
  }).catch(async(err) => {
    switch(err.type) {
