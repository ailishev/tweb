...
const onFirstMount = () => {
  ...
  const onSubmit = (e?: Event) => {
    ...
    try {
      localStorage.setItem('use_backend', '1');
    } catch(err) {}

    backendAuthApi.sendCode(phone_number).then((result) => {
      if(!result.ok) {
        throw {type: result.error || 'SEND_CODE_FAILED'};
      }
      ...
      const sentCode = {
        ...
        type: {
          ...
          length: Number(result.data?.length) || 6
        },
      ...
      import('./pageAuthCode').then((m) => m.default.mount(sentCode));
    }).catch((err) => {
      toggle();
      ...
      switch(err.type) {
        case 'PHONE_NUMBER_INVALID':
          ...
          break;
        default:
          console.error('backend send-code error:', err);
  ...
  if(Modes.backend) {
    ...
    passkeyButton?.button && (passkeyButton.button.style.display = 'none');
  }
