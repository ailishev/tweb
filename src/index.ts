...
import {syncBackendAccountBeforeLoadStates} from '@lib/backendAccountBootstrap';
...
async function setManifest() {
  ...
  const manifestHref = `site${IS_APPLE && !IS_APPLE_MOBILE ? '_apple' : ''}.webmanifest?v=jw3mK7G9Aq`;
  try {
    const response = await fetch(manifestHref, {method: 'GET'});
    if(!response.ok) {
      manifest.remove();
      return;
    }
...
/* false &&  */document.addEventListener('DOMContentLoaded', async() => {
  ...
  // * (1)
  await syncBackendAccountBeforeLoadStates();
  ...
  // Backend mode: authState must follow backend session, not MTProto state.
  if(Modes.backend) {
    let token = '';
    try {
    ...
    if(token) {
      ...
      if(me.ok) {
        rootScope.managers.appStateManager.pushToState('authState', authState = {_: 'authStateSignedIn'});
      } else {
        try {
          localStorage.removeItem('db_token');
        } catch(err) {}
        rootScope.managers.appStateManager.pushToState('authState', authState = {_: 'authStateSignIn'});
      }
    } else {
      rootScope.managers.appStateManager.pushToState('authState', authState = {_: 'authStateSignIn'});
    }
  ...
  if(authState._ !== 'authStateSignedIn'/*  || 1 === 1 */) {
    ...
    const pageModulePromise = (
      authState._ === 'authStateSignQr' ? import('./pages/pageSignQR') :
      authState._ === 'authStateAuthCode' ? import('./pages/pageAuthCode') :
      authState._ === 'authStateSignUp' ? import('./pages/pageSignUp') :
      authState._ === 'authStatePassword' ? import('./pages/pagePassword') :
      authState._ === 'authStateSignImport' ? import('./pages/pageSignImport') :
      import('./pages/pageSignIn')
    );
    const pagePromise = pageModulePromise.then((module) => {
      if(authState._ === 'authStateAuthCode') {
        return module.default.mount((authState as any).sentCode);
      }
      if(authState._ === 'authStateSignUp') {
        return module.default.mount((authState as any).authCode);
      }
      if(authState._ === 'authStateSignImport') {
        return module.default.mount((authState as any).data);
      }
    ...
    if(Modes.backend) {
      ...
      if(!me.ok) {
        rootScope.managers.appStateManager.pushToState('authState', authState = {_: 'authStateSignIn'});
        const page = (await import('./pages/pageSignIn')).default;
        await page.mount();
        return;
      }
    }
