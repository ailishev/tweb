...
export default class ConnectionStatusComponent {
  ...
  public construct(
    ...
    if(Modes.backend) {
      this.applyBackendTransportPlaceholder();
      return;
    }
  ...
  /** Custom backend: no Telegram DC / MTProto connection bar — search stays usable during WS reconnect. */
  private applyBackendTransportPlaceholder() {
    ...
    this.inputSearch.setPlaceholder('Search');
    this.inputSearch.toggleLoading(false);
  }
