...
function createUsernameRow(
  ...
  if(isLink && icon === 'link_paid') row.container.classList.add('is-paid');
  media.append(Icon(icon));

  return row;
}

export class UsernameRow {
  constructor(
    isLink?: boolean,
    icon: Icon = 'limit_link',
    color?: string,
    ...
    return createUsernameRow(isLink, icon, color, titleRight, subtitleRight) as any;
  }
}

// Keep full Row API in types, but avoid runtime `extends Row` (cyclic TDZ in dev).
export interface UsernameRow extends Row {}
...
export default class UsernamesSection extends SettingSection {
  ...
  constructor(options: {
    ...
    const changeActive = (row: UsernameRow, active: boolean) => {
